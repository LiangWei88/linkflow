...
export class BlockRepository {
  ...
  getByDocId(docId: string): Block[] {
    const stmt = db.prepare(
      'SELECT * FROM blocks WHERE docId = ? ORDER BY parentId NULLS FIRST, blockOrder ASC'
    )
  ...
  getMaxOrder(docId: string, parentId: string | null = null): number {
    ...
    if (parentId === null) {
      stmt = db.prepare(
        'SELECT MAX(blockOrder) as maxOrder FROM blocks WHERE docId = ? AND parentId IS NULL'
      )
      result = stmt.get(docId)
    } else {
      stmt = db.prepare(
        'SELECT MAX(blockOrder) as maxOrder FROM blocks WHERE docId = ? AND parentId = ?'
      )
